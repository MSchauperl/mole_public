...
class ChemBLDataModule(CrossEnvDataModule):
    ...
    def __init__(
        ...
        # New parameter for fold-based splits
        ...
        **kwargs,
        ...
        Args:
            ...
            validation_split: Fraction for validation set (used only if split_indices_path is None)
            test_split: Fraction for test set (used only if split_indices_path is None)
            max_targets: Maximum number of targets to use
            min_target_activity: Minimum activity count per target
            max_samples: Maximum number of samples to use (for testing)
            split_indices_path: Path to pickle file containing train/val split indices
    ...
    def _load_chembl_smiles(self) -> List[str]:
        ...
        if isinstance(smiles_data, list):
            ...
            if self.max_samples is not None and len(smiles_data) > self.max_samples:
                print(
                    f"Limiting dataset from {len(smiles_data)} to {self.max_samples} samples"
                )
    ...
    def _load_split_indices(self) -> Dict[str, List[int]]:
        """Load train/validation split indices from pickle file."""
        if self.split_indices_path is None:
            return None

        print(f"Loading split indices from {self.split_indices_path}")
        with open(self.split_indices_path, "rb") as f:
            splits = pickle.load(f)

        if not isinstance(splits, dict) or "train" not in splits or "val" not in splits:
            raise ValueError(
                f"Expected dict with 'train' and 'val' keys, got {type(splits)}"
            )

        print(
            f"Loaded splits: {len(splits['train'])} train, {len(splits['val'])} validation"
        )
        return splits
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit" or stage is None:
            ...
            # Check if we should use fold-based splits
            if self.split_indices_path is not None:
                ...
                val_start_idx = len(train_smiles)  # Offset for validation set
                self.val_dataset = ChemBLDatasetWithOffset(
                    smiles=val_smiles, offset=val_start_idx, **val_dataset_kwargs
                )

                # Store test smiles for later (empty for now)
                self._test_smiles = []
                self._test_start_idx = len(train_smiles) + len(val_smiles)

            else:
                # Use original random split approach
                # Split data
        ...
        if stage == "test" and hasattr(self, "_test_smiles"):
            if len(self._test_smiles) == 0:
                print("No test set available (using fold-based splits)")
                return
            ...
            self.test_dataset = ChemBLDatasetWithOffset(
                smiles=self._test_smiles, offset=self._test_start_idx, **dataset_kwargs
            )
...
class ChemBLDatasetWithOffset(ChemBLDataset):
    ...
    def __getitem__(self, idx: int):
        ...
        data = super(ChemBLDataset, self).__getitem__(
            idx
        )  # Skip ChemBLDataset.__getitem__
        ...
        try:
            ...
            print(
                f"Warning: Failed to add ChemBL targets for molecule {idx} (offset {self.offset}): {e}"
            )
